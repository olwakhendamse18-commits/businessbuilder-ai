...
### 16.2 Phase 2B.2C.2 database-only worker canary

The worker canary is a bounded control-plane proof stored separately from voice state. `worker_canary_probes` has one row per logical worker role and contains only `worker_role`, a random bounded `probe_id`, exact expected commit, bounded status, request/expiry/claim/completion timestamps, an internal random claim token and lease, completed commit, safe result code, and update timestamp. It has no relationship to `agent_voice_sessions`, creates no fake upstream call identity, stores no user, voice-session, call, transcript, SDP, URL, API-key, database-location, or operator data, and exposes no public HTTP or browser route.

A canary request is allowed only when the exact role has recent valid `last_poll_at` and `last_success_at` heartbeat evidence and its validated deployed commit matches the requested exact 40-character ASCII hexadecimal commit. Request, active-row normalization, and the one-row upsert occur atomically. A non-expired `pending` or `in_progress` probe may be reused only when its validated expected commit exactly matches the new request. An active different-commit probe is never overwritten or associated with the new request; the safe conflict returns no probe identity. Terminal or expired rows may be replaced by an explicitly requested new probe. Same-commit and cross-commit request concurrency is serialized per role. Probe, token, role, result-code, and commit inputs use exact bounded ASCII validation without stripping or truncation.

The existing worker processes at most one canary per iteration after bounded voice maintenance and before one generic background job. Claiming uses a fresh short transaction: SQLite uses `BEGIN IMMEDIATE`; PostgreSQL locks the heartbeat and canary rows with `FOR UPDATE`. A claim requires the exact current heartbeat instance owner, matching stored and deployed commits, valid and recent `last_poll_at` and `last_success_at`, pending status, and a future overall expiry. A matching but stale or malformed heartbeat cannot claim. The claim commits an internal random claim token and 30-second lease and closes the connection. The worker then calls an explicit deterministic in-memory synthetic-step boundary, and only after that succeeds does it open a separate fresh guarded finalization transaction. No transaction spans claim, synthetic processing, and finalization. SQLite and mocked PostgreSQL tests freeze the exact claim-close, synthetic-step, and finalization ordering, including cross-commit concurrency and ownership-loss behavior.

Finalization requires the exact role, probe, current heartbeat instance, expected/deployed commit, in-progress status, internal claim token, unexpired claim lease, and unexpired overall probe. Success clears the token and lease and stores `worker_database_round_trip_verified`. A stale token or superseded instance cannot overwrite newer state. Heartbeat ownership loss is the existing fencing event: the superseded worker stops before further canary, voice, generic, research, monitoring, heartbeat, or external work. An expired claim lease returns an otherwise-live probe to pending for the new heartbeat owner; overall expiry becomes terminal `expired`; `completed` and `failed` remain terminal absent an explicit new request. Normalization is idempotent and needs no cleanup service.

The canary path performs no HTTP request, DNS lookup, OpenAI operation, Realtime creation, hangup, microphone action, or API-key lookup. It is independent of `VOICE_RUNTIME_ENABLED`, so disabled voice creation does not disable heartbeat, canary, or existing cleanup. Worker cadence remains voice maintenance, at most one canary, then one generic job; generic work still runs after canary completion or ordinary canary failure, busy work has no added sleep, and fully idle iterations retain `WORKER_POLL_SECONDS`. Only one generic worker instance remains the approved topology.

Authorized deployment shells may run `flask verify-worker-canary --expected-commit <40-character-sha> --wait-seconds <0-300>`. The default wait is 90 seconds and polling uses fresh connections no more often than once per second. Output is limited to `status`, fixed role, aggregate worker-commit state, aggregate canary age, and `result=verified|not_verified`; it never exposes probe, token, instance, timestamp, database, service, user/call, secret, or raw-error data. An active different-commit conflict is reported as pending/not-verified with matching live-worker commit state, unknown canary age, exit `3`, no probe identity, and no polling of the foreign probe. Exit codes are `0` verified, `2` worker missing/stale, `3` timed out or active-conflict pending, `4` commit mismatch, `5` commit unknown, `6` database/schema unavailable, `7` expired, and `8` failed.

A verified result proves that the deployed worker observed synthetic work through the same visible database, retained heartbeat ownership, atomically claimed the bounded probe, closed the claim transaction, and completed a separate guarded finalization. It does not prove OpenAI availability or a successful hangup. Live PostgreSQL verification, deployment verification, and a separately authorized real upstream hangup smoke test remain rollout gates. No deployment-file change occurs in this phase, and production voice activation remains **NO-GO**.
